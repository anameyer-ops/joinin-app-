exports.handler = async (event) => {
  ...
  try{
    if(event.httpMethod==='GET'){
      ...
      const path=`/rest/v1/pdis?${base}${filter}&select=*,colaboradores(nome,cargo,area)&order=created_at.desc`;
      const d=await sb(id?`/rest/v1/pdis?id=eq.${id}&select=*`:path);
    ...
    if(event.httpMethod==='POST'){
      ...
      const d=await sb('/rest/v1/pdis',{method:'POST',body:JSON.stringify(payload)});
      return{statusCode:201,headers,body:JSON.stringify(Array.isArray(d)?d[0]:d)};
    }
    if(event.httpMethod==='PUT'){const b=JSON.parse(event.body);const d=await sb(`/rest/v1/pdis?id=eq.${id}`,{method:'PATCH',body:JSON.stringify(b)});return{statusCode:200,headers,body:JSON.stringify(Array.isArray(d)?d[0]:d)};}
    if(event.httpMethod==='DELETE'){await sb(`/rest/v1/pdis?id=eq.${id}&user_id=eq.${user.id}`,{method:'DELETE'});return{statusCode:200,headers,body:JSON.stringify({ok:true})};}
